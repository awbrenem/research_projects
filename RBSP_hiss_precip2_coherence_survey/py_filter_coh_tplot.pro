...
pro py_filter_coh_tplot
    ...
    if KEYWORD_SET(args) then begin
        ...
        ;combo = 'IX'
        combo = 'I_PeakDet_thb_peem_ptotQ'
;        pre = '2'    ;OBSOLETE VARIABLE
;        fspc = 0 ;obsolete
    ...
    varname = 'coh_'+combo
    get_data,varname,data=dd,dlim=dlim,lim=lim


    result = mean_filter(dd.y,meanfilterwidth,meanfilterheight,/nan,/geometric)
    varname = 'coh_'+combo+'_meanfilter'
    dat = {x:dd.x,y:result,v:dd.v}
    store_data,varname,data=dat,dlim=dlim,lim=lim

    varname = 'phase_'+combo
    get_data,varname,data=dd,dlim=dlim,lim=lim
    result = mean_filter(dd.y,meanfilterwidth,meanfilterheight,/nan,/geometric)
    varname = 'phase_'+combo+'_meanfilter'
    dat = {x:dd.x,y:result,v:dd.v}
    store_data,varname,data=dat,dlim=dlim,lim=lim
    zlim,varname,0,90,0
    ...
    fntmp = datapath + folder + '/' + combo + '_meanfilter'
    tplot_save,savevars,filename=fntmp
