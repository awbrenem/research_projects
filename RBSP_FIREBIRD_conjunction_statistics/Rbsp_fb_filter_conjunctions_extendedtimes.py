...
def read_conjunction_microbursts_file(path,fn):


    header = ["Tstart","Tend","uB_maxamp"]
    file_name = path + fn
    df = pd.read_csv(file_name, header=None, skiprows=1, delim_whitespace=True, names=header, 
                    na_values=[99999.9, 9999.99, 999.99])
...
def add_subplot(df_surviving_list, lrange, mltrange, dlrange, dmltrange, xkey, ykey, ax_number):
    ...
    nbins = np.floor((np.log10(binsmax) - np.log10(binsmin)) * 10)

    xi,yi,xi2,yi2,zi = get_kde_values(df_fin, xkey, ykey, nbins)
    ax[ax_number[0],ax_number[1]].set_title(str(len(df_surviving_list))+" events; "+str(len(df_fin[xkey]))+" data points;  L="+str(lrange[0])+"-"+str(lrange[1])+"  MLT="+str(mltrange[0])+"-"+str(mltrange[1])+"  dL="+str(dlrange[0])+"-"+str(dlrange[1])+"  dMLT="+str(dmltrange[0])+"-"+str(dmltrange[1]))
    ax[ax_number[0],ax_number[1]].pcolormesh(xi2, yi2, zi.reshape(xi.shape), shading='auto')
    ax[ax_number[0],ax_number[1]].set_yscale("log")
    ax[ax_number[0],ax_number[1]].set_xscale("log")
    ax[ax_number[0],ax_number[1]].set_xlabel(xkey)
    ax[ax_number[0],ax_number[1]].set_ylabel(ykey)
...
if __name__ == "__main__":
    ...
    df_b4 = read_conjunction_file(path, "RBSPb_FU4_conjunction_values_hr.txt")

    #Read in the microburst amplitudes from Shumko's code 
    path = "/Users/abrenema/Desktop/code/Aaron/github/research_projects/RBSP_Firebird_microburst_conjunctions_all/RBSP_FB_final_conjunction_lists/"
    uB_b4 = read_conjunction_microbursts_file(path, "RBSPb_fu4_conjunction_microbursts.txt")




    #Plot the "microburst" amplitudes. Make sure to remove values that are saturated. 
    plt.plot(df_b4["colHR"])
    plt.plot(df_b4["colS"])
    ...
    for i in range(10): print(df_b4["Tstart"][i]  + '  ' + uB_b4["Tstart"][i])
    ...
    #xkey = "SpecBAvg_lb"
    xkey = "FBKmaxB"
    ykey = "colHR"   #Detrended values representing "microbursts"
    #ykey = "colS"
