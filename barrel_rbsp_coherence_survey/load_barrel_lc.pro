...
;Ex, load_barrel_lc,['2T','2W'],type='rcnt'
...
;TWO DIFFERENT VERSIONS WITHIN TDAS:
;HERE'S THE ONE MY CODE IS TRYING TO CALL WHEN I USE THE PYTHON SCRIPT:
;/Users/aaronbreneman/Desktop/code/Aaron/RBSP/TDAS_trunk_svn/general/spedas_tools/spd_download/
;  spd_download_file.pro
;HERE'S THE ONE THAT SEEMS TO WORK.
;/Users/aaronbreneman/Desktop/code/Aaron/RBSP/TDAS_trunk_svn/general/misc



pro load_barrel_lc,payloads,type=type,level=level


  if ~keyword_set(level) then level = 'l2'
;  if ~keyword_set(ver) then ver = 'v05'
  ;else begin
  ;   if float(ver) lt 10 then ver = 'v0' + ver else ver='v'+ver
  ;endelse
  ...
  ;Find version number
  numtmp = strmid(payloads[0],0,1)
  if numtmp eq '1' or numtmp eq '2' or numtmp eq '3' then ver = 'v05'
  if numtmp eq '4' then ver = 'v06'



  path = 'http://barreldata.ucsc.edu/data_products/'
  remote_data_dir = path + ver + '/l2/'
  if type eq 'ephm' then remote_data_dir = path + ver + '/l2/'
  local_data_dir = '/Users/aaronbreneman/data/rbsp/barrel/'
  ...
  for j=0,n_elements(payloads)-1 do begin
     rcnt = 0. & rcntt = 0D & lshell2 = 0. & lshell2t = 0D & lshell6 = 0.
     lshell6t = 0D & mlt2 = 0. & mlt2t = 0D & mlt6 = 0. & mlt6t = 0D
     alt = 0. & altt = 0D & lat = 0. & latt = 0D & long = 0.
     longt = 0D & lc1 = 0. & lc1t = 0D
     lc1a = 0. & lc1at = 0D & lc1b = 0. & lc1bt = 0D & lc1c = 0. & lc1ct = 0D
     lc2 = 0. & lc2t = 0D & lc3 = 0. & lc3t = 0D & lc4 = 0. & lc4t = 0D




     mspc = replicate(0,1,48) & mspct = 0D
     sspc = replicate(0,1,256) & sspct = 0D
     ...
     for q=0,ndays-1 do begin
        ...
        file = 'bar_'+payloads[j]+'_'+level+'_'+type+'_'+date+'_'+ver+'.cdf'
        relpathnames = dirpath[j] + file


;****No download ver
;        file_loaded = spd_download(remote_path=remote_data_dir+dirpath[j],remote_file=file,$
;        local_path=local_data_dir+dirpath[j],/last_ver,/no_download)

        file_loaded = spd_download(remote_path=remote_data_dir+dirpath[j],remote_file=file,$
        local_path=local_data_dir+dirpath[j],/last_ver)
...
;           store_data,['L_Kp?','MLT_Kp?_T89c'],/delete
        ...
        if type eq 'fspc' then begin
           ...
           if mission eq '2' or mission eq '3' or mission eq '4' then begin
...
;;--------------------------------------------------
     ...
     if type eq 'fspc' then begin
        ...
        if nelem ne 1 then begin
           ...
           if mission eq '2' or mission eq '3' or mission eq '4' then begin
              ...
              options,'FSPC1a_'+payloads[j],'ytitle','FSPC1a_'+payloads[j]+'!C25-48keV'
              options,'FSPC1b_'+payloads[j],'ytitle','FSPC1b_'+payloads[j]+'!C48-95keV'
              options,'FSPC1c_'+payloads[j],'ytitle','FSPC1c_'+payloads[j]+'!C95-180keV'
           endif else begin

              lc1 = lc1[1:nelem-1]
              lc1t = lc1t[1:nelem-1]

              store_data,'FSPC1_'+payloads[j],data={x:lc1t,y:lc1}
              options,'FSPC1_'+payloads[j],'ytitle','FSPC1_'+payloads[j]+'!C25-180keV'
           endelse
           ...
           options,'FSPC2_'+payloads[j],'ytitle','FSPC2_'+payloads[j]+'!C180-550keV'
           options,'FSPC3_'+payloads[j],'ytitle','FSPC3_'+payloads[j]+'!C550-840keV'
           options,'FSPC4_'+payloads[j],'ytitle','FSPC4_'+payloads[j]+'!C840-1500keV'
     ...
     if type eq 'mspc' then begin
        ...
        store_data,'MSPC_'+payloads[j],data={x:mspct,y:mspc,v:mspcv}
        options,'MSPC_'+payloads[j],'spec',1
        options,'MSPC_'+payloads[j],'ytitle','MSPC_'+payloads[j]+'!CkeV'
     ...
     if type eq 'sspc' then begin
        ...
        store_data,'SSPC_'+payloads[j],data={x:sspct,y:sspc,v:sspcv}
        options,'SSPC_'+payloads[j],'spec',1
        options,'SSPC_'+payloads[j],'ytitle','SSPC_'+payloads[j]+'!CkeV'
     endif
