;Crib sheet for aaron_map_with_t04.pro. This program takes GSM coordinates
...
;interpolate the sc coord onto a higher cadence
;times = dindgen(86400./10.) + time_double(storm_start)
;t0 = time_double(storm_start)
;t1 = time_double('2016-01-20/22:00:00')
;nelem = 86400./10.
;times = (t1-t0)*dindgen(nelem)/(nelem-1) + time_double(storm_start)
;tinterpol_mxn,gsm_coord,times,newname=gsm_coord
...
aaron_map_with_tsy,model,storm_start,dur,'RBSP'+sc,'rbsp'+sc+'_state_pos_gsm'


tplot,['RBSPa!CL-shell-'+model,$
  'RBSPa!Cequatorial-foot-MLT!C'+model,$
  'RBSPa!Cnorth-foot-MLT!C'+model,$
  'RBSPa!Csouth-foot-MLT!C'+model]

  vars = ['RBSPa!CL-shell-'+model,$
    'RBSPa!Cequatorial-foot-MLT!C'+model,$
    'RBSPa!Cnorth-foot-MLT!C'+model,$
    'RBSPa!Csouth-foot-MLT!C'+model]

    if model eq 't96' then tplot_save,vars,filename='~/Desktop/rbspa_tsy96_mapping_20160120'
    if model eq 't04' then tplot_save,vars,filename='~/Desktop/rbspa_tsy04_mapping_20160120'
